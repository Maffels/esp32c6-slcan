...
#include "Arduino.h"
// Minimal ESP32-C6 USB-CAN / SLCAN adapter
// Adapted from https://github.com/mintynet/esp32-slcan (esp32-twai-can)
// FOR USE WITH Arduino ESP32 Board package 3.x
// Pins for this hardware: MCP2562 on ESP32-C6
// TX=GPIO1, RX=GPIO2
// Tested target: 500 kbit/s with Kvaser Leaf Light v2 + python-can slcan

// USB CDC on boot enabled (in board settings if available)
// 3.3V logic
...
void setup() {
  ...
  while(!Serial);
  t_config = TWAI_TIMING_CONFIG_500KBITS();  // default 500k (S6). Change via S command before O.
  delay(1000);
} // setup()
